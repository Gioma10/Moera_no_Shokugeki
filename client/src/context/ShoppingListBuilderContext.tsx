...
const ShoppingListBuilderContext =
  createContext<ShoppingListBuilderContextType | null>(null);
    ...
    for (const ing of recipe.ingredients) {
      ...
      if (isQB) {
        ...
        const existing = result.find(
          (i) =>
            i.ingredient.toLowerCase() === ing.ingredient.toLowerCase() &&
            i.unit === ing.unit,
        );
...
export function ShoppingListBuilderProvider({
  children,
}: {
  ...
  const mergedIngredients = useMemo(
    () => mergeIngredients(selectedRecipes),
    [selectedRecipes],
  );
...
export function useShoppingListBuilder() {
  ...
  if (!context)
    throw new Error(
      "useShoppingListBuilder must be used inside ShoppingListBuilderProvider",
    );
  return context;
}
